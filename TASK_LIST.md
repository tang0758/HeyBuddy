...
- [x] 深度优化圆角渲染，解决 Windows 窗口底层直角层问题 (v3.4)。
- [x] 实现完全自定义的 Flutter 阴影，替代系统矩形阴影。

## 阶段五：发布与维护 (进行中) 🚀
- [x] 导出最终的 Flutter Windows Release 版本 (.exe)。
- [ ] 更新 GitHub 仓库说明与部署指南。
- [ ] 尝试在 MacOS 环境下编译并运行测试。

## 阶段六：多终端工具兼容拓展 (计划中) 🚧
- [ ] 兼容 Claude Code CLI (通过 PTY 输出正则匹配拦截 `[y/n]` 提示)。
- [ ] 兼容 Codex CLI 交互授权。
- [ ] 抽象底层通信协议，使拦截器逻辑与具体的 CLI 解耦。

## 阶段七：网络配对与配置系统优化 (计划中) 🚧
- [ ] 废弃脆弱的 `resolv.conf` IP 探测，实现基于配置文件的 IP/Port 绑定。
- [ ] (探索) 局域网自动发现机制 (mDNS/Bonjour) 或扫码配对机制。
- [ ] 支持自定义监听端口与安全鉴权密钥 (Token Auth)，防止局域网内恶意调用。
